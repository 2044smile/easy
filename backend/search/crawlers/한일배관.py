...
from base import crawler_settings
...
def get_product_info(product_name):
    ...
    while True:
        ...
        filtered_items = [item for item in items if product_name in item.get('title', '')]
        
        if not items:  # 결과가 없으면 종료
            break

        # filtered_items = filter(lambda item: product_name in item.get('title', ''), items)
    ...
    filename = f'search/crawlers/data/product_data_{datetime.now().strftime("%Y%m%d")}_{product_name}.csv'
    df.to_csv(filename, index=False, encoding='utf-8-sig')
    
    return all_products


get_product_info("수도계량기")